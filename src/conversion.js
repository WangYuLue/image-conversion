...
(function (factory) {
    ...
    methods.urltoImage = function (url) {
        return new Promise((resolve, reject) => {
            ...
            img.onerror = () => reject('urltoImage(): Image failed to load, please check the image URL');
            img.src = url;
        })
    };
    /**
    ...
    methods.dataURLtoImage = function (dataURL) {
        return new Promise((resolve,reject) => {
            const img = new Image();
            img.onload = () => resolve(img);
            img.onerror = () => reject('dataURLtoImage(): dataURL is illegal');
            img.src = dataURL;
        });
    };
    ...
    methods.compress = async function (file, config={}) {
        ...
        if (typeof config === 'number') {
            config = Object.assign({quality: config}) ;
        }
        ...
        for (let x = 1; x <= 7; x++) {
            ...
            if (x === 7) {
                if (resultSize.max < CalculationSize || resultSize.min > CalculationSize) {
                    compressDataURL = [compressDataURL, ...tempDataURLs].filter(i=>i).sort((a, b) =>
